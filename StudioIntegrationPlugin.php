...
class StudioIntegrationPlugin extends GenericPlugin
{
    ...
    public function displayTemplateHook(string $hookName, array $params): bool
    {
        ...
        if (!$context || !$user || !in_array($request->getRequestedPage(), ['workflow', 'dashboard'], true)) {
            return false;
        }
        $submissionId = $this->resolveSubmissionId($request);
        if ($submissionId < 1 || !$this->userCanAccessSubmission($user, $context, $submissionId)) {
            return false;
        }
        ...
        $claims = [
            ...
            'context' => ['externalId' => (string)$contextId, 'type' => 'journal', 'path' => $context->getPath()],
            'submission' => ['externalId' => (string)$submissionId],
            'actor' => ['externalId' => (string)$user->getId()],
            'scope' => ['metadata.read', 'contributors.read', 'files.read', 'manuscript.read', 'manuscript.write', 'revision.write'],
            'iat' => $now,
            'exp' => $now + $ttl,
        ...
        $launchUrl = $studioUrl . '/integrations/ojs/launch?' . http_build_query($token, '', '&', PHP_QUERY_RFC3986);
        $templateMgr = $params[0];
        $pluginBase = $request->getBaseUrl() . '/' . $this->getPluginPath();
        $config = json_encode(['launchUrl' => $launchUrl, 'label' => __('plugins.generic.studioIntegration.openInStudio')], JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE | JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT);
        if ($config === false) {
            return false;
        }
        $templateMgr->addHeader('studioIntegrationConfig', '<script>window.OMI_STUDIO_INTEGRATION=' . $config . ';</script>', ['contexts' => ['backend']]);
        $templateMgr->addJavaScript('studioIntegration', $pluginBase . '/js/studioIntegration.js', ['contexts' => ['backend']]);
        $templateMgr->addStyleSheet('studioIntegration', $pluginBase . '/css/studioIntegration.css', ['contexts' => ['backend']]);
    ...
    private function resolveSubmissionId($request): int
    {
        foreach (['workflowSubmissionId', 'submissionId', 'id'] as $key) {
            $value = (int)$request->getUserVar($key);
            if ($value > 0) {
                return $value;
            }
        }
        foreach ($request->getRequestedArgs() as $arg) {
            if (ctype_digit((string)$arg) && (int)$arg > 0) {
                return (int)$arg;
            }
        }
        return 0;
