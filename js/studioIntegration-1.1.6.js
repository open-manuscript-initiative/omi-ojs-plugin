(function () {
  ...
  function unwrapJsonMessage(data) {
    if (!data || typeof data !== 'object') return null;

    if (typeof data.launchUrl === 'string') return data;
  ...
  async function fetchLaunchUrl(config, submissionId) {
    ...
    if (config.mode === 'review') endpoint.searchParams.set('mode', 'review');
    ...
    var rawData = null;
    try { rawData = await response.json(); } catch (_error) { rawData = null; }
    var data = unwrapJsonMessage(rawData);

    if (!response.ok || (rawData && rawData.status === false)) {
      var errorSource = data && data.error ? data : rawData;
      var detail = errorSource && errorSource.error && errorSource.error.message
        ? errorSource.error.message
        : 'HTTP ' + response.status;
      throw new Error(detail);
    }
    if (!data || typeof data.launchUrl !== 'string' || !data.launchUrl) {
  ...
  function createLauncher(config, submissionId) {
    ...
    var label = config.label || (config.mode === 'review' ? 'Open in Studio for Review' : 'Open in Studio');
    ...
    button.addEventListener('click', async function (event) {
      ...
      try {
        ...
        window.setTimeout(function () {
          if (document.body.contains(button)) setState(button, 'ready', label);
        }, 5000);
      }
    }, true);
  ...
  function scheduleMount() { window.setTimeout(mount, 0); }
